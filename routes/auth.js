...
router.get("/activate/:token", async (req, res) => {
  ...
  if (user.email === email) {
    ...
    return res.redirect(
      "https://www.google.com/search?q=cosas+buenas&sxsrf=ALiCzsYrUGxfQtl2BKeIOsqeWaUCPiP-8w:1653642411044&source=lnms&tbm=isch&sa=X&ved=2ahUKEwjfpPuDqv_3AhXOnXIEHfh_BlEQ_AUoAnoECAIQBA&biw=1064&bih=1065&dpr=0.9#imgrc=9t0RvrDPmarPBM"
    );
  }
});
...
router.post("/recovery", async (req, res) => {
  ...
  if (!user) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, redirect: false, msg: "Something goes wrong" });
    }
  }
});
